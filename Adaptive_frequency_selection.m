...
clear
close all
file_path =  'E:\windows\学术\汇报\33 开题\针对方案一的程序\FEM\PHM 2012\zhenghaiyang-phm-ieee-2012-data-challenge-dataset-master\phm-ieee-2012-data-challenge-dataset\Full_Test_Set\Bearing1_6\';% 文件夹路径
% 全寿命振动信号
...
plot(csv_acc_data(:,5));title('水平振动信号');
...
A=A';
nodes=[A];   %第3层的节点号
ord=wpfrqord(nodes);  %小波包节点号重排，ord是重排后小波包系数索引构成的矩阵　如3层分解的[1;2;4;3;7;8;6;5]
nodes_ord=nodes(ord); %重排后的小波包节点号
for i=1:2^D_layers
...
for i=1:2^D_layers %这里先重构之后再排序是没问题的，因为cfs{i}是和rex3(:,i)有相似性的
