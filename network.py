...
class ProbAttention(nn.Module):
    ...
    def _prob_QK(self, Q, K, sample_k, n_top):  # n_top: c*ln(L_q)
        ...
        M_top = M.topk(n_top, sorted=False)[1]  # 可在不进行自动排序的情况下选出前n_top个最大值
    ...
    def _get_initial_context(self, V, L_Q):
        ...
        if not self.mask_flag:
            # V_sum = V.sum(dim=-2)
            V_sum = V.mean(dim=-2)  # 按列求均值
            # aa = V_sum.unsqueeze(-2)
            contex = V_sum.unsqueeze(-2).expand(B, H, L_Q, V_sum.shape[-1]).clone()
        else:  # use mask
            assert (L_Q == L_V)  # requires that L_Q == L_V, i.e. for self-attention only
            contex = V.cumsum(dim=-2)  # 按列cumsum
        return contex
...
class AttentionLayer(nn.Module):  #可选择调用哪一种attention
...
class Net(nn.Module):
    def __init__(self, inchannels=1, size=128):
        ...
        self.fc1 = nn.Linear(256 * 16 * 16, 2560)
        self.fc2 = nn.Linear(2560, 768)
